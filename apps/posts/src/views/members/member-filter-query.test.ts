...
describe('member-filter-query', () => {
    ...
    it('drops past-direction syntax on a future-only field', () => {
        // `subscriptions.current_period_end` only advertises `in-the-next`,
        // never `in-the-last`. A hand-crafted URL that pairs the past
        // direction with this field has no UI representation, so we drop it
        // rather than smuggle it in as an absolute-date predicate the user
        // can't reach through normal interaction.
        expect(parseMemberFilter('subscriptions.current_period_end:>=now-7d', 'UTC')).toEqual([]);
    });
